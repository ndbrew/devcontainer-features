...
ASDFVERSION=${ASDFVERSION:-"latest"}
PLUGIN=${PLUGIN:-""}
...
install_via_asdf() {
	...
	if cat /etc/os-release | grep "ID_LIKE=.*alpine.*\|ID=.*alpine.*" ; then
        check_alpine_packages curl git ca-certificates jq
	elif cat /etc/os-release | grep  "ID_LIKE=.*debian.*\|ID=.*debian.*"; then
		check_packages curl git ca-certificates jq
	fi
...
EOF
		...
		if [ "$ASDFVERSION" == "latest" ]; then
			ASDFVERSION=$(curl -sL https://api.github.com/repos/asdf-vm/asdf/releases/latest | jq -r ".tag_name")
		fi
		...
		su - "$_REMOTE_USER" <<EOF
			git clone --depth=1 \
					...
					"https://github.com/asdf-vm/asdf.git" --branch $ASDFVERSION $_REMOTE_USER_HOME/.asdf 2>&1
